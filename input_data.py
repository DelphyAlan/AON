...
def decode(serialized_example):
    example_features = tf.io.parse_single_example(
        serialized_example,
        # Defaults are not specified since both keys are required.
        features={
            'image/transcript': tf.io.FixedLenFeature([], dtype=tf.string),
...
def read_tfrecord_use_queue_runner(filename, batch_size=32):
    ...
    image = tf.image.resize(image, [100, 100]) / 128.0 - 1  # normalize to [-1, 1)
